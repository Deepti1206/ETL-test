---
title: "Exploratory_Analysis_using_R_Impact_of_city_health_services_on_public"
...
miss.data <- data.frame(colSums(is.na(data_health))/nrow(data_health) )
...
miss.data$Missing_Perc <- round(miss.data$Missing_Perc*100, 5)


ggplot(miss.data[miss.data$Missing_Perc != 0 , ] , aes(x = reorder(Col_name , - Missing_Perc) , y = Missing_Perc))+ geom_bar(stat='identity')+geom_text(aes(label=Missing_Perc), vjust=0)
...
```{r laod datasummary1 , echo=TRUE}
summary(select(cities, "csmoking", "sleep", "lpa", "binge"))
```
```{r laod data43, echo=TRUE}
